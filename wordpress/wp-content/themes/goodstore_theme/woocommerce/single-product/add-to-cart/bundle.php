						...
						<div class="variations">
							...
							$loop = 0; foreach ( $attributes[ $bundled_item_id ] as $name => $options ) { $loop++; ?>
								...
								<select id="<?php echo esc_attr( sanitize_title( $name ) . '_' . $bundled_item_id ); ?>" name="attribute_<?php echo sanitize_title( $name ); ?>">
									<option value=""><?php echo __( 'Choose an option', 'jawtemplates' ) ?>&hellip;</option>
									<?php
										if( is_array( $options ) ) {
								...
								if ( sizeof( $attributes[ $bundled_item_id ] ) == $loop ) {
									echo '<a class="reset_variations" href="#reset_' . $bundled_item_id .'">'.__( 'Clear selection', 'jawtemplates' ).'</a>';
								}
	...
	<?php do_action('woocommerce_before_add_to_cart_button'); ?>
		...
		<div class="bundle_wrap" style="display:none;">
			...
			<div class="bundle_button">
				...
				<button type="submit" class="button alt"><?php echo apply_filters( 'single_add_to_cart_text', __( 'Add to cart', 'jawtemplates' ), $product->product_type ); ?></button>
			</div>
