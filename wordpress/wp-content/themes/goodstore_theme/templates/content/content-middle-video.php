...
<article id="post-<?php the_ID(); ?>"  <?php post_class(array('element', $class, 'col-lg-4', 'content-middle', 'format-video')); ?>   
         sort_name="<?php echo $sort . (StrToLower(get_the_title())); ?>"  
         sort_date="<?php echo get_the_time("Y-m-d H:i:s") . $sort_num; ?>" 
         sort_rating="<?php echo $sort_num . $rating; ?>" 
         sort_popular="<?php echo $sort_num . get_comments_number();     //if(jwOpt::get_option('fbcomments_switch','0')=='0'){echo get_comments_number(); }else{echo jwFacebook::get_fb_comments_count(get_the_ID()); }       ?>"
         sort_category="<?php echo $sort . (isset($terms[0]->slug)) ? $terms[0]->slug : '' ?>">
    <div class="box ">
        <div class="image">
            <?php
            ...
            if (!jwUtils::has_post_thumbnail()) {
                ...
                if (isset($video->thumbnails['thumbnail_medium'])) {
                    echo '<img src="' . $video->thumbnails['thumbnail_medium'] . '"  width = "' . (jwUtils::get_size('4') - 40) . '" height="193" / >';
                }
            } else {
