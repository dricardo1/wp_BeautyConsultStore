    ...
    class jwMetabox {
        ...
        function build_meta_box($post, $metabox) {
            ...
            if (isset($this->meta_box['type']) && $this->meta_box['type'] == 'page_builder') {
                ...
                echo '<div id="item-{{entity.id}}" class="builder-element s_{{entity.size}} jb-{{entity.name}}" ng-repeat="(i,entity) in workspace" ng-include src="\'temp-build_build_enitiy\'"></div>'; //\'+entity.name
                echo '<div class="clear"></div>';
                echo '</div>';
                echo '</div>';
                ...
                $unserialized_presets = @unserialize(jwOpt::get_options('builder'));
                if ($unserialized_presets != false) {
                    $pressets = str_replace('\'', '`', $unserialized_presets);
                } else {
                    $pressets = $unserialized_presets;
                }
                $unserialized_elements_pressets = @unserialize(jwOpt::get_options('builder_element'));
                if ($unserialized_elements_pressets != false) {
                    $elements_pressets = str_replace('\'', '`', $unserialized_elements_pressets);
                } else {
                ...
                echo '        <input class="pb_element_change"  type="text" value="{{entity.title}}" ng-model="entity.title">';
                echo '    </div>';
                echo '    </div>';
                echo '    <div class="zaves"></div>';
