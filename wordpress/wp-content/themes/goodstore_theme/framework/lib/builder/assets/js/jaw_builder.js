 ...
 * @since GS
 */
var jawBuilder = angular.module('jawBuilder', ['ui.bootstrap', 'ui.sortable', 'colorpicker.module', 'jaw.gallerypicker', 'jaw.simplemediapicker']);
...
var builder = function($scope, $dialog, $timeout,$window) {
    ...
    $scope.control_edit = function(item) {
        ...
        if (open_editor == false) {
            ...
            //angular.bootstrap(jQuery('#jaw-builder-popup'), ['jawEditor']);
            $scope.opts = {
                backdrop: true,
                keyboard: true,
                backdropClick: false,
                //templateUrl: template_url + '/lib/builder/metabox-editor.php?code=' + item.name, // OR: templateUrl: 'path/to/view.html',
                templateUrl: ajaxurl+'?action=jaw_builder_editor&code=' + item.name,//template_url + '/lib/builder/metabox-editor.php?code=' + item.name, // OR: templateUrl: 'path/to/view.html',
    ...
    $scope.add_preset = function() {
        ...
        actual_edit = jQuery.extend(true, {}, new_element.metabox);
        $scope.opts = {
            backdrop: true,
            keyboard: true,
            backdropClick: true,
            templateUrl: ajaxurl+'?action=jaw_builder_editor&code=' + 'add_preset',//templateUrl: template_url + '/lib/builder/metabox-editor.php?code=add_preset',
            controller: 'editor_dialog'
        };
        var d = $dialog.dialog($scope.opts);
        d.open().then(function(result) {
    ...
    /*
     ...
     keyboard: true,
     backdropClick: true,
     templateUrl: template_url + '/lib/builder/metabox-editor.php?code=bug', // OR: templateUrl: 'path/to/view.html',
     controller: 'editor_dialog'
     };
     var d = $dialog.dialog($scope.opts);
     d.open().then(function(result) {
     ...
     ajaxurl,
     {
     'action': 'pb_bug_report',
     'bug': debug_log,
     'comment':  result.comment
     },
     function() {
     $scope.openMessageBox('Thank you', 'for repport', 'ok');
     }
     );
     }
     }); 
